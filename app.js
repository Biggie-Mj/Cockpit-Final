...
function normalize(s){
  ...
  out.ambiences.forEach(a=>{a.name=String(a.name||'').trim();a.spotifyUrl=String(a.spotifyUrl||'').trim()});
...
const genericDefs={
  ambience:{eyebrow:'AMBIANCE',title:'Ambiance',collection:'ambiences',fields:[{name:'name',label:'Nom de l’ambiance',type:'input'},{name:'spotifyUrl',label:'Lien Spotify (facultatif)',type:'input',span:2,required:false}]},
...
function renderLibrary(){
  ...
  if(tab==='ambiences')html=state.ambiences.map((a,i)=>{const complete=ambienceIsComplete(a);return `<article class="library-card ambience-library-card ${complete?'':'ambience-incomplete'}"><header><div><h3>${esc(a.name)}</h3><div class="subtitle">${complete?'PRÊTE':'À COMPLÉTER'}</div></div></header><footer><span class="ambience-order-controls"><button type="button" class="ghost ambience-order-btn" data-move-ambience="${esc(a.id)}|-1" ${i===0?'disabled':''} title="Monter" aria-label="Monter ${esc(a.name)}">${uiIcon('fleche_haut','ui-icon')}</button><button type="button" class="ghost ambience-order-btn" data-move-ambience="${esc(a.id)}|1" ${i===state.ambiences.length-1?'disabled':''} title="Descendre" aria-label="Descendre ${esc(a.name)}">${uiIcon('fleche_bas','ui-icon')}</button></span>${ambiencePlayButton(a)}<button data-edit-generic="ambience:${esc(a.id)}">Modifier</button></footer></article>`}).join('');
...
function bindLibraryActions(){
  ...
  $$('[data-move-ambience]').forEach(b=>b.onclick=()=>{const [id,direction]=b.dataset.moveAmbience.split('|');moveAmbience(id,Number(direction))});
...
async function copyIllustrationRecord(item,changes={}){const type=item.type||item.blob?.type||'image/jpeg',buffer=await item.blob.arrayBuffer(),blob=new Blob([buffer],{type});return {...item,...changes,type,size:blob.size,blob}}
...
async function renameIllustration(id,name){const item=illustrationRecords.find(x=>x.id===id),nextName=String(name||'').trim();if(!item)return;if(!nextName)return toast('Donne un nom à l’illustration');try{const updated=await copyIllustrationRecord(item,{name:nextName});await putIllustrations([updated]);$('#illustrationRenameDialog').close();editingIllustrationId=null;await renderIllustrations();toast('Illustration renommée')}catch(err){console.error('Renommage illustration',err);toast('Impossible de renommer cette illustration')}}
...
function ambienceIsComplete(a){return !!a&&validAmbienceUrl(a.spotifyUrl)}
function ambiencePlayButton(a){if(!ambienceIsComplete(a))return '';return `<button class="ambience-play" data-play-ambience="${esc(a.id)}" aria-label="Ouvrir ${esc(a.name)} dans Spotify" title="Ouvrir dans Spotify"><img src="music-note.png" alt=""></button>`}
function moveAmbience(id,direction){const index=state.ambiences.findIndex(a=>a.id===id),next=index+Math.sign(Number(direction)||0);if(index<0||next<0||next>=state.ambiences.length||next===index)return;commit(()=>{const [item]=state.ambiences.splice(index,1);state.ambiences.splice(next,0,item)},'Ordre des ambiances modifié');if($('#ambienceDrawer')?.open)renderAmbienceDrawer()}
...
function renderAmbienceDrawer(){
  $('#ambienceDrawerList').innerHTML=state.ambiences.length?state.ambiences.map((a,i)=>{const complete=ambienceIsComplete(a);return `<div class="ambience-row ${complete?'':'ambience-incomplete'}"><span class="ambience-row-label"><strong>${esc(a.name)}</strong>${complete?'':'<small>À compléter</small>'}</span><span class="ambience-drawer-actions"><span class="ambience-order-controls"><button type="button" class="ghost ambience-order-btn" data-move-ambience="${esc(a.id)}|-1" ${i===0?'disabled':''} title="Monter" aria-label="Monter ${esc(a.name)}">${uiIcon('fleche_haut','ui-icon')}</button><button type="button" class="ghost ambience-order-btn" data-move-ambience="${esc(a.id)}|1" ${i===state.ambiences.length-1?'disabled':''} title="Descendre" aria-label="Descendre ${esc(a.name)}">${uiIcon('fleche_bas','ui-icon')}</button></span>${ambiencePlayButton(a)}</span></div>`}).join(''):'<p class="empty-mini">Ajoute tes ambiances dans Composants → Ambiances.</p>';bindAmbiencePlayback();$('#ambienceDrawer').querySelectorAll('[data-move-ambience]').forEach(b=>b.onclick=()=>{const [id,direction]=b.dataset.moveAmbience.split('|');moveAmbience(id,Number(direction))});
}
